Use Master
Go
If(db_id('MalumaBB')Is Not Null)
Drop DataBase MalumaBB
Create DataBase MalumaBB
Go
...
Insert Cliente Values('4766746','Byron Gualip','kissi','Cuyotenango', '96687349')
Go
...
As Begin
	If(Exists(Select * From Cliente Where Dni=@DPI))
		Set @Mensaje='Los Datos del Cliente ya Existen.'
	Else Begin
		Insert Cliente Values(@DPI,@Apellidos,@Nombres,@Direccion,@Telefono)
...
As Begin
	If(Not Exists(Select * From Cliente Where Dpi=@DPI))
...
@Datos Varchar(80)
As Begin
	Select IdCliente,DPI,Apellidos,Nombres,Direccion,Telefono 
...
Insert Producto Values(1,'Gaseosa','Pepsi',5,4.90,5.90,'12/12/2018')
Go
...
As Begin
	If(Not Exists(Select * From Empleado Where Dni=@Dni))
		Begin
		Insert Empleado Values(@IdCargo,@Dpi,@Apellidos,@Nombres,@Sexo,@FechaNac,@Direccion,@EstadoCivil)
			Set @Mensaje='Registrado Correctamente Ok.'
		End
	Else Begin
	If(Exists(Select * From Empleado Where Dpi=@Dpi))
		Begin
		Update Empleado Set IdCargo=@IdCargo,Dni=@Dpi,Apellidos=@Apellidos,Nombres=@Nombres,Sexo=@Sexo,
...
As Begin
	...
	Begin
		Insert DetalleVenta Values(@IdProducto,@IdVenta,@Cantidad,@PrecioUnitario,@Iva,@SubTotal)
			Set @Mensaje='Registrado Correctamente.'
	End
...
Go
Insert Empleado Values(1,'34004387','Daniel Alecio','Malumababy','M','11/01/1996','San Andres','S')
Go
Insert Usuario Values(1,'malumababy','123456')
Go
